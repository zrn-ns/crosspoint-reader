...
- お気に入り作家の登録・管理（作家一覧でアクションメニューから登録、ホーム画面から直接アクセス）
...
- WebUIからSleep画面用画像のアップロード・管理が可能
- ブラウザ側でディザリング・BMP変換を行い、デバイスに直接転送
- スリープ画面に当月カレンダーをオーバーレイ表示（設定で配置位置を選択可能）
- WiFi接続時にNTP自動同期（JST対応）

### その他

- X3の4階調グレースケール対応（テキスト・画像のアンチエイリアス品質向上）
- Xteink X3 ハードウェアサポート
- カーニング・リガチャ対応（本家 v1.2.0 由来）
- 脚注ナビゲーション（本家 v1.2.0 由来）
- X3の側面ボタンマッピング修正（X3/X4のハードウェア差異に対応）
- ファイル・フォルダの再帰的削除対応
- ホーム画面表示時の空ディレクトリ自動削除
